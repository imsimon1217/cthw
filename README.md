...
原有供應商提供的校園導賞團連結已移除。除非校方擁有 VR 原始檔及授權，否則不建議把供應商內容直接搬到學校 server。新版已預留校園環境區，日後可放校方自家相片、影片或授權 VR 檔案。
...
如果網站放在支援 PHP 的學校 server，後台會優先呼叫 `api/`，把老師更新的內容寫入 server database。若只用靜態方式打開，後台會暫存在同一瀏覽器的 localStorage，方便校方試用流程；亦可在後台下載新的 `site-content.json`。
...
4. 如果用 MySQL，把 `api/config.php` 入面的 `database` 改成 MySQL host、database、username、password。
5. 正式上線前請更改老師登入密碼及 hash。
