...
- `data/site-content.json`：首頁、分類樹及分類封面可更新內容資料
...
新版保留現有網站可見的主要導覽結構：關於趙小、學與教、多元體驗、eClass、Facebook，以及最新消息、學校資訊、活動花絮、加入趙小、招標、課程發展、語文學習、STEAM、個人成長、其他學習經歷、課外活動、走出課室及星級導師等內容分類。八個主分類及其子分類由同一分類樹產生；分類頁、文章清單及文章詳情會優先使用已導入的真實校園／活動相片作封面。
...
如果網站放在支援 PHP 的學校 server，後台會優先呼叫 `api/`，把老師更新的內容寫入 server database，文章與分類封面圖片會上載到 `assets/uploads/`。若只用靜態方式打開，後台會把圖片縮小後暫存在同一瀏覽器的 localStorage，方便校方試用流程；亦可在後台下載新的 `site-content.json`。
